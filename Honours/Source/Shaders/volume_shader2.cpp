#include "volume_shader2.h"
#include "../Managers/camera.h"
VolumeShaderTWO::VolumeShaderTWO(void){
}
VolumeShaderTWO::~VolumeShaderTWO(void){
}
bool VolumeShaderTWO::Initialize(ID3D11Device* device, HWND hwnd){
	bool result;
	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"Shader/Volume2/volume2.vs", L"Shader/Volume2/volume2.ps");
	if(!result){
		return false;
	}
	return true;
}
bool VolumeShaderTWO::Render(ID3D11DeviceContext* device_context, int index_count_, D3DXMATRIX world_matrix, D3DXMATRIX viewMatrix, 
						  D3DXMATRIX projection_matrix, ID3D11ShaderResourceView* volumeTexture, D3DXVECTOR4 camera){
	// Set the shader parameters that it will use for rendering.
	bool result = SetShaderParameters(device_context, world_matrix, viewMatrix, projection_matrix, volumeTexture, camera);
	if(!result){
		return false;
	}
...
bool VolumeShaderTWO::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vs_filename, WCHAR* ps_filename){
	HRESULT result;
	...
	volume_buffer_desc.ByteWidth = sizeof(CameraBufferType);
...
bool VolumeShaderTWO::SetShaderParameters(ID3D11DeviceContext* device_context, D3DXMATRIX world_matrix, D3DXMATRIX viewMatrix, 
									   D3DXMATRIX projection_matrix, ID3D11ShaderResourceView* volumeTexture, D3DXVECTOR4 camera){
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mapped_resource;
	MatrixBufferType* data_ptr;
	CameraBufferType* data_ptr_two;
	...
	// Get a pointer to the data in the constant buffer.
	data_ptr_two = (CameraBufferType*)mapped_resource.pData;
	// Copy the lighting variables into the constant buffer.
	data_ptr_two->CameraPositionTS = camera;
	...
	data_ptr_two->Inverse = ObjectToTextureSpaceMatrix;
	// Unlock the constant buffer.
...
void VolumeShaderTWO::RenderShader(ID3D11DeviceContext* device_context, int index_count_){
