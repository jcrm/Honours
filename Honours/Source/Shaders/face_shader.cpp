...
bool FaceShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vs_filename, WCHAR* ps_filename){
	HRESULT result;
	...
	D3D11_BUFFER_DESC scale_buffer_desc;
	D3D11_SAMPLER_DESC sampler_desc;
	// Compile the vertex shader code.
	result = D3DX11CompileFromFile(vs_filename, NULL, NULL, "FaceVS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
	...
	result = D3DX11CompileFromFile(ps_filename, NULL, NULL, "FacePS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
	...
	scale_buffer_desc.Usage = D3D11_USAGE_DYNAMIC;
	scale_buffer_desc.ByteWidth = sizeof(ScaleBufferType);
	scale_buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	scale_buffer_desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	scale_buffer_desc.MiscFlags = 0;
	scale_buffer_desc.StructureByteStride = 0;
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	result = device->CreateBuffer(&scale_buffer_desc, NULL, &scale_buffer_);
	if(FAILED(result)){
		return false;
	}
	return true;
...
void FaceShader::RenderShader(ID3D11DeviceContext* device_context, int index_count){
