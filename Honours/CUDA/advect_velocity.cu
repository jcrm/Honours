...
__global__ void cuda_kernel_advect_velocity(float *output, float*input, Size size, int identifier){ 
	...
	for(z_iter = 0; z_iter < size.depth_; ++z_iter){ 
		if((x_iter - 1 >= 0 && x_iter + 1 < size.width_) && (y_iter - 1 >= 0 && y_iter + 1 < size.height_) && (z_iter - 1 >= 0 && z_iter + 1 < size.depth_)){
			...
			float temp_X_1 = field_left_up[x_identifier_] +((field_left_down[x_identifier_]-field_left_up[x_identifier_])*0.5f);
			float temp_Y_1 = field_left_up[y_identifier_] +((field_left_down[y_identifier_]-field_left_up[y_identifier_])*0.5f);
			float temp_Z_1 = field_left_up[z_identifier_] +((field_left_down[z_identifier_]-field_left_up[z_identifier_])*0.5f);

			float temp_X_2 = field_right_up[x_identifier_] +((field_right_down[x_identifier_]-field_right_up[x_identifier_])*0.5f);
			float temp_Y_2 = field_right_up[y_identifier_] +((field_right_down[y_identifier_]-field_right_up[y_identifier_])*0.5f);
			float temp_Z_2 = field_right_up[z_identifier_] +((field_right_down[z_identifier_]-field_right_up[z_identifier_])*0.5f);

			float temp_X_3 = field_left_up_back[x_identifier_] +((field_left_down_back[x_identifier_]-field_left_up_back[x_identifier_])*0.5f);
			float temp_Y_3 = field_left_up_back[y_identifier_] +((field_left_down_back[y_identifier_]-field_left_up_back[y_identifier_])*0.5f);
			float temp_Z_3 = field_left_up_back[z_identifier_] +((field_left_down_back[z_identifier_]-field_left_up_back[z_identifier_])*0.5f);

			float temp_X_4 = field_right_up_back[x_identifier_] +((field_right_down_back[x_identifier_]-field_right_up_back[x_identifier_])*0.5f);
			float temp_Y_4 = field_right_up_back[y_identifier_] +((field_right_down_back[y_identifier_]-field_right_up_back[y_identifier_])*0.5f);
			float temp_Z_4 = field_right_up_back[z_identifier_] +((field_right_down_back[z_identifier_]-field_right_up_back[z_identifier_])*0.5f);
