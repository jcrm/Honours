...
__global__ void cuda_kernel_advect_velocity(float *output, float*input, Size size){ 
	...
	for(z_iter = 0; z_iter < size.depth_; ++z_iter){ 
		if((x_iter - 1 >= 0) && (y_iter - 1 >= 0) && (z_iter - 1 >= 0)){
			...
			float temp_X_1 = field[x_identifier_] +(-field[x_identifier_]*0.5f);
			float temp_Y_1 = field[y_identifier_] +(-field[y_identifier_]*0.5f);
			float temp_Z_1 = field[z_identifier_] +(-field[z_identifier_]*0.5f);

			float temp_X_2 = fieldDown[x_identifier_] +(-fieldDown[x_identifier_]*0.5f);
			float temp_Y_2 = fieldDown[y_identifier_] +(-fieldDown[y_identifier_]*0.5f);
			float temp_Z_2 = fieldDown[z_identifier_] +(-fieldDown[z_identifier_]*0.5f);

			float temp_X_3 = fieldBack[x_identifier_] +(-fieldBack[x_identifier_]*0.5f);
			float temp_Y_3 = fieldBack[y_identifier_] +(-fieldBack[y_identifier_]*0.5f);
			float temp_Z_3 = fieldBack[z_identifier_] +(-fieldBack[z_identifier_]*0.5f);

			float temp_X_4 = fieldDownBack[x_identifier_] +(-fieldDownBack[x_identifier_]*0.5f);
			float temp_Y_4 = fieldDownBack[y_identifier_] +(-fieldDownBack[y_identifier_]*0.5f);
			float temp_Z_4 = fieldDownBack[z_identifier_] +(-fieldDownBack[z_identifier_]*0.5f);
