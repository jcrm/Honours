		...
		if(x_iter +1 < size_WHD.x && x_iter - 1 > 0){
			if(y_iter + 1 < size_WHD.y && y_iter - 1 > 0){
				if(z_iter + 1 < size_WHD.z && z_iter - 1 > 0){
					...
					unsigned char *pLeft = pressure + (z_iter*pitch_slice) + (y_iter*pitch) + (4*(x_iter-1));
					unsigned char *pRight = pressure + (z_iter*pitch_slice) + (y_iter*pitch) + (4*(x_iter+1));
					unsigned char *pDown = pressure + (z_iter*pitch_slice) + ((y_iter-1)*pitch) + (4*x_iter); 
					unsigned char *pUp = pressure + (z_iter*pitch_slice) + ((y_iter+1)*pitch) + (4*x_iter); 
					unsigned char *pTop = pressure + ((z_iter-1)*pitch_slice) + (y_iter*pitch) + (4*x_iter);
					unsigned char *pBottom = pressure + ((z_iter+1)*pitch_slice) + (y_iter*pitch) + (4*x_iter);
					unsigned char *cell_velocity = velocity + (z_iter*pitch_slice) + (y_iter*pitch) + (4*x_iter);
