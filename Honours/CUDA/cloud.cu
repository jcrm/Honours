...
__global__ void cuda_kernel_project(float*pressure, float* velocity, float* advect, Size size, int pressure_index){
...
extern "C"
void cuda_fluid_project(void *pressure, void *velocityInput, void* advect, Size size, int pressure_index){
	cudaError_t error = cudaSuccess;

	dim3 Db = dim3(16, 16);   // block dimensions are fixed to be 256 threads
	dim3 Dg = dim3((size.width_+Db.x-1)/Db.x, (size.height_+Db.y-1)/Db.y);

	cuda_kernel_project<<<Dg,Db>>>((float *)pressure, (float *)velocityInput, (float*)advect, size, pressure_index);
