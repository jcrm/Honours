...
__global__ void cuda_kernel_initial(unsigned char *input, float3 size_WHD, size_t pitch, size_t pitch_slice, float value){ 
	...
	for(z_iter = 0; z_iter < size_WHD.z; ++z_iter){ 
		...
		unsigned char *cell_value = input + (z_iter*pitch_slice) + (y_iter*pitch) + (PIXEL_FMT_SIZE * x_iter);
		...
		if(x_iter +1 >= size_WHD.x){
			cell_value[0] = signed int(value*10);
			cell_value[1] = signed int(0.5f*value);
			cell_value[2] = signed int(-value);
			cell_value[3] = signed int(value);
		}
	}
}
extern "C"
